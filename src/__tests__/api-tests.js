...
describe("Env Checker API", () => {
    ...
    describe("loadEnvFiles", () => {
        ...
        it("should skip missing env files", () => {
            fs.existsSync.mockImplementation((p) => p === ".env1" ? true : false);
            dotenv.config.mockReturnValue({ parsed: { A: "1" } });
    ...
    describe("loadExampleFile", () => {
        it("should read and return example file content", () => {
            fs.existsSync.mockReturnValue(true);
            fs.readFileSync.mockReturnValue("VAR1=\nVAR2=");
        ...
        it("should throw if example file does not exist", () => {
            ...
            expect(() => loadExampleFile(".env.example")).toThrow("Example file not found: .env.example");
        });
    });
    describe("checkEnv", () => {
        it("should return missing, extra, type errors, and sensitive warnings", () => {
            ...
            expect(result.typeErrors).toContain('Variable "PORT" should be a number but got "abc"');
            expect(result.sensitiveWarnings).toContain("Potential sensitive data found: TOKEN");
        });
    });
});
