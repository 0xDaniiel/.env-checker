...
program
    ...
    if (options.generate) {
        try {
            ...
            console.log(chalk.green(`✅ Generated ${examplePath} from ${envPaths[0]}`));
            process.exit(0);
        }
        catch (err) {
    ...
    let exampleContent;
    try {
        exampleContent = loadExampleFile(path.resolve(process.cwd(), options.example));
    }
    catch (err) {
    ...
    const missing = Object.keys(exampleVars).filter((key) => !(key in mergedEnvVars));
    const extra = Object.keys(mergedEnvVars).filter((key) => !(key in exampleVars));
    if (options.json) {
        console.log(JSON.stringify({ missing, extra }, null, 2));
    }
    else {
        if (missing.length) {
            console.warn(chalk.yellow(`⚠️ Missing variables: ${missing.join(", ")}`));
        }
        if (extra.length) {
    ...
    if (typeValidationResult.errors.length) {
        typeValidationResult.errors.forEach((err) => console.error(chalk.red(`❌ ${err}`)));
        if (options.ci)
            process.exit(1);
    }
    else {
    ...
    if (sensitiveWarnings.length) {
        sensitiveWarnings.forEach((warn) => console.warn(chalk.yellow(`⚠️ ${warn}`)));
        if (options.ci)
            process.exit(1);
    }
    if (options.ci &&
