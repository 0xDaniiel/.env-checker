...
function parseTypeAnnotations(exampleContent) {
    const typeMap = {};
 ...
 */
export function validateTypes(envVars, exampleContent) {
    const errors = [];
        ...
        switch (expectedType.toLowerCase()) {
            case "number":
                if (isNaN(Number(value))) {
                    errors.push(`Variable "${key}" should be a number but got "${value}"`);
                }
                break;
            case "boolean":
                if (!["true", "false"].includes(value.toLowerCase())) {
                    errors.push(`Variable "${key}" should be a boolean ("true" or "false") but got "${value}"`);
                }
                break;
            case "string":
            default:
